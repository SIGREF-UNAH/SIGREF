...
export const EmployeesListForm = () => {
...
const [modal, contextHolder] = Modal.useModal();
 ...
 const employees: Employee[] =
  data?.map((p: any, index: number) => ({
    id: p.id?.value ?? index + 1,
    name: p.name?.[0]?.text?.value ?? "Sin nombre", // <-- aquí
    email: p.telecom?.find((t: any) => t.system?.value === "email")?.value?.value ?? "Sin correo",
    position: p.qualification?.[0]?.code?.text?.value ?? "No especificado",
    area: p.address?.[0]?.text?.value ?? "Sin área",
    status: p.active?.value ? "Activo" : "Inactivo",
  })) ?? [];
  ...
  content: `¿Estás seguro de que deseas eliminar a ${employee.name}? Esta acción no se puede deshacer.`,
  okText: "Eliminar",
  okType: "danger",
  cancelText: "Cancelar",
  onOk: async () => {
